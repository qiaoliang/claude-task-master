			...
			console.log(
				boxen(
					chalk.white.bold(`Task ${newTaskId} Created Successfully`) +
						...
						'\n' +
						chalk.cyan(
							`1. Run ${chalk.yellow(`ztm show ${newTaskId}`)} to see complete task details`
						) +
						'\n' +
						chalk.cyan(
							`2. Run ${chalk.yellow(`ztm set-status --id=${newTaskId} --status=in-progress`)} to start working on it`
						) +
						'\n' +
						chalk.cyan(
							`3. Run ${chalk.yellow(`ztm expand --id=${newTaskId}`)} to break it down into subtasks`
						),
