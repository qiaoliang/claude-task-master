...
To optimize LLM context usage, you can control which Task Master MCP tools are loaded using the `ZH_TASK_MASTER_TOOLS` environment variable. This is particularly useful when working with LLMs that have context limits or when you only need a subset of tools.
...
You can also use `"lean"` as an alias for `"core"`. If `ZH_TASK_MASTER_TOOLS` is not set, core tools are loaded by default.
...
```json
{
  "mcpServers": {
    "ztm-ai": {
      ...
      "args": ["-y", "ztm-ai"],
      "env": {
        "ZH_TASK_MASTER_TOOLS": "get_tasks,next_task,set_task_status,update_subtask",
        "ANTHROPIC_API_KEY": "your_key_here"
      }
    }
  }
